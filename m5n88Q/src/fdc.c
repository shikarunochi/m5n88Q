...
int disk_insert(int drv, const char *filename, int img, int readonly)
{
  ...
  */
  ...
  M5SerialPrintln("DISK OPEN SUCCESS");
    ...
    */
    ...
    while (!exit_flag)
    {
      ...
      case D88_BAD_IMAGE: /* このイメージは壊れている */
        ...
        M5SerialPrintln("DISK IMAGE BROKEN?");
        drive[drv].detect_broken_image = TRUE;
        exit_flag = TRUE;
        break;
  ...
  if (img < 0 || img >= drive[drv].image_nr)
  {
    ...
    M5SerialPrintln("DISK SET EMPTY");
    drive_set_empty(drv);
  }
  else
  {
  ...
  M5SerialPrintln("DISK IMAGE READ SUCCESS");
  return 0;
}
...
static int fdc_check_unit(void)
{
  ...
  { /* WRITE ID        の場合        */
    ...
    {
      fdc.wait += CLOCK_TRACK() * (drive[drv].sec_nr - drive[drv].sec) / drive[drv].sec_nr;
    }
    ...
    { /* このセクタには IAM がある         */
      ...
      { /* READ / WRITE の場合       */
        ...
        { /* IDR 一致時        */
          ...
          else if (sec_buf.status == STATUS_MA_MD && /* (D)DAM なし   */
