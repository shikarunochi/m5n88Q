...
#define set_ALU_comp()           \
  ...
  {                              \
    ALU_comp.l = 0;              \
    if ((ALU2_ctrl & 0x01) == 0) \
      ALU_comp.l |= 0x000000ff;  \
    if ((ALU2_ctrl & 0x02) == 0) \
      ALU_comp.l |= 0x0000ff00;  \
    if ((ALU2_ctrl & 0x04) == 0) \
      ALU_comp.l |= 0x00ff0000;  \
  } while (0)
#else
#define set_ALU_comp()           \
  do                             \
...
static void sio_set_intr_base(void)
{
      ...
      ((sys_ctrl & 0x20) || (sys_ctrl & 0x08)))
  {
    ...
    { /* CMT 指定時 */
      ...
      { /* 通常は、*/
        if (sys_ctrl & 0x10)
          sio_bps = 1200; /* I/O 30h:bit4=1 で 1200bps */
        else
          sio_bps = 600; /*             =0 で  600bps */
      }
      else
      {
...
static T_SUSPEND_W suspend_pc88main_work[] =
    {
...

        {
            TYPE_INT,
            &ALU_buf.l,
        }, /*  TYPE_CHAR, ALU_buf.c[0]-[3] ?  */
        {
            TYPE_INT,
            &ALU_comp.l,
        }, /*  TYPE_CHAR, ALU_comp.c[0]-[3] ? */
